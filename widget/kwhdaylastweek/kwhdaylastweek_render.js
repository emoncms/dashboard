...
function kwhdaylastweek_widgetlist()
{
  var widgets =
  {
    "kwhdaylastweek":
  ...
  var decimalsDropBoxOptions = [        // Options for the type combobox. Each item is [typeID, "description"]
	...
	var sizeoptions = [
					[14, "18"], // set size 18 to the top position to be the default value for creating new kwhdaylastweek widgets otherwise size 40 would be always the default
					[13, "40"],
					[12, "36"],
					[11, "32"],
					[10, "28"],
  ...
  var alignmentOptions = [
	...
	addOption(widgets["kwhdaylastweek"], "colour",   "colour_picker",  _Tr("Colour"),     _Tr("Colour used for display"),      []);
	addOption(widgets["kwhdaylastweek"], "font",     "dropbox",  _Tr("Font"),     _Tr("Font used for display"),      fontoptions);
	addOption(widgets["kwhdaylastweek"], "fstyle",   "dropbox", _Tr("Font style"), _Tr("Font style used for display"),    fstyleoptions);
	addOption(widgets["kwhdaylastweek"], "fweight",  "dropbox", _Tr("Font weight"), _Tr("Font weight used for display"),    fweightoptions);
	addOption(widgets["kwhdaylastweek"], "size",   	"dropbox", _Tr("Size"), _Tr("Text size in px to use"),    sizeoptions);
	addOption(widgets["kwhdaylastweek"], "align",    "dropbox", _Tr("Alignment"), _Tr("Alignment"), alignmentOptions);
	addOption(widgets["kwhdaylastweek"], "timeout",  "value",   _Tr("Timeout"),    _Tr("Timeout without feed update in seconds (empty is never)"),   []);
	addOption(widgets["kwhdaylastweek"], "errormessagedisplayed",    "value",  _Tr("Error Message"),   _Tr("Error message displayed when timeout is reached"),   []);

	return widgets;
}

function draw_kwhdaylastweek(kwhdaylastweek,font,fstyle,fweight,width,height,prepend,val,append,colour,decimals,size,align,errorCode,errorMessage)
{
    ...
    if (errorCode === "1")
    {
        kwhdaylastweek.html(errorMessage);
    }
    else
    {
        kwhdaylastweek.html(prepend+val+append);
    }
}

function kwhdaylastweek_draw()
{
    $(".kwhdaylastweek").each(function(index)
    {
        var kwhdaylastweek = $(this);
        ...
        var errorTimeout = kwhdaylastweek.attr("timeout");
        ...
        var size = kwhdaylastweek.attr("size");

        var decimals = kwhdaylastweek.attr("decimals");

        if (decimals===undefined) {decimals = -1};


        // backwards compatibility
        var unitend = kwhdaylastweek.attr("unitend");
        var units = kwhdaylastweek.attr("units");
        
        // new options
        var prepend = kwhdaylastweek.attr("prepend");
        var append = kwhdaylastweek.attr("append");
        ...
        draw_kwhdaylastweek(
            kwhdaylastweek,
            kwhdaylastweek.attr("font"),
            kwhdaylastweek.attr("fstyle"),
            kwhdaylastweek.attr("fweight"),
            kwhdaylastweek.width(),
            kwhdaylastweek.height(),
            prepend,
            val,
            append,
            kwhdaylastweek.attr("colour"),
            kwhdaylastweek.attr("decimals"),
            kwhdaylastweek.attr("size"),
            kwhdaylastweek.attr("align"),
            errorCode,
            errorMessage
        );
    });
